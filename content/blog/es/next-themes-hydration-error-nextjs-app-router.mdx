...
abstract: "Soluciona los ‘hydration mismatch’ provocados por next-themes en proyectos Next 13/14 con App Router. Aprende a crear un ThemeProvider compatible con SSR y a consumirlo desde cualquier componente client con el hook `useTheme`."
...
Con **next-themes** y **App Router** (Next 13/14) esto sucede porque el componente `<ThemeProvider />` lee `window.matchMedia`, `localStorage`, y otras APIs exclusivas del navegador durante el renderizado del servidor.  
El resultado usual es una advertencia en la consola como:
