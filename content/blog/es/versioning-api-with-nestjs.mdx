...
abstract: "Aprende a habilitar y organizar el versionado de tus endpoints REST en NestJS usando distintos esquemas (URI, encabezado, media type), buenas prácticas de estructura de carpetas y ejemplos concretos con controladores y servicios."
...
async function bootstrap() {
  ...
  // Habilitar versionado en la URI: /api/v{version}/{route}
...
export class ProductsController {
  ...
  @Get()
  findAll() {
    // Nueva estructura o campos adicionales en V2
    return this.service
      .findAll()
...
app.enableVersioning({
  ...
  key: "v", // Accepta: application/json;v=2
  defaultVersion: "1",
});
```
...
app.enableVersioning({
  ...
  factory: (request) => {
    // extraer la versión desde la cookie o parámetro de consulta
